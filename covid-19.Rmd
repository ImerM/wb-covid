...
dead_graph <- dead %>% group_by(Country) %>% mutate(days_since_start = difftime(Date, min(Date), units = "days")) %>% ggplot(aes(days_since_start, value, color = Country)) + geom_line(size = 2) + theme_wsj() + theme(legend.text = element_text(size = 12), plot.title = element_text(size = 14, hjust = 0.5), plot.subtitle = element_text(size = 10)) + scale_color_brewer(palette = "Set2") + labs(title = "Number of recoveries from COVID19 in Western Balkans", caption ="Data source:https://github.com/CSSEGISandData/COVID-19/", subtitle = "By: Imer Muhovic @imerm1", x = "Days since first death")
```
...
```{r pressure, plotly=TRUE,fig.width=10,fig.height=8, echo=FALSE}

interactive_confirmed_graph <- confirmed_it_100 %>% group_by(Country) %>% filter(value > 100) %>% mutate(days_since_start = difftime(Date, min(Date), units = "days")) %>% filter(days_since_start < 21) %>% ggplot(aes(days_since_start, value, color = Country)) + geom_line(size = 2) + scale_y_log10() + theme_clean() + scale_color_brewer(palette = "Set2") + labs(title = "Number of cases of COVID19 in Western Balkans since 100th confirmed case", caption ="Data source:https://github.com/CSSEGISandData/COVID-19/", x = "Days since 100th confirmed case, Italy added for comparison")
ggplotly(interactive_confirmed_graph)
```

## Overall COVID19 State In Western Balkans

```{r interactive-totals, plotly=TRUE,fig.width=10,fig.height=8}
